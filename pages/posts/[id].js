...
import postsPageInitialPaths4c0b9Resource from '../../resources/posts-page-initial-paths-4c0b9'
import postsPageInitialPropsB6f2fResource from '../../resources/posts-page-initial-props-b6f2f'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - National Integration Specialist</title>
          <meta
            property="og:title"
            content="Posts1 - National Integration Specialist"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths4c0b9Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsB6f2fResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
